...
___
`ReplaceRegexSubstring(origin: str, regex: str, substring: str) -> str`

**Описание:** Заменяет все вхождения регулярного выражения в строке на подстроку.

**Аргументы:**
* _origin_ – обрабатываемая строка;
* _regex_ – регулярное выражение для поиска подстрок;
* _substring_ – вставляемая подстрока.

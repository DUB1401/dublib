...
class HttpxConfig:
    ...
    def __init__(self, auto_user_agent: bool = True):
        ...
        if auto_user_agent:
            self.__Headers = {"User-Agent": UserAgentGenerator().chrome}
...
class WebRequestor:
    ...
    def __GetProxy(self) -> dict | None:
        ...
        if len(self.__Proxies) > 0:
            ...
            if Proxy["login"] != None and Proxy["password"] != None: Auth = Proxy["login"] + ":" + Proxy[
                "password"] + "@"
            ...
            if type(self.__Config) == RequestsConfig:
                ...
                Proxy = {
                    Proxy["protocol"]: Proxy["protocol"].replace("https", "http") + "://" + Auth + Proxy["host"] + ":" +
                                       Proxy["port"]
                }
            ...
            if type(self.__Config) == HttpxConfig:
                # Создание объекта прокси.
                Proxy = {
                    Proxy["protocol"] + "://": Proxy["protocol"].replace("https", "http") + "://" + Auth + Proxy[
                        "host"] + ":" + Proxy["port"]
                }
    ...
    def __InitializeChrome(self):
        ...
        self.__Browser = self.selenium.webdriver.Chrome(
            service=self.Service.Service(self.ChromeDriverManager.ChromeDriverManager().install()),
    ...
    def __httpx_GET(self, url: str, params: dict | None = None, headers: dict | None = None,
        ...
        # Ответ.
        Response = self.__Client.get(url, params=params, headers=headers, cookies=cookies,
                                     follow_redirects=self.__Config.redirecting)

        return Response

    def __httpx_POST(self, url: str, params: dict | None = None, headers: dict | None = None,
                     cookies: dict | None = None, data: any = None, json: dict | None = None) -> httpx.Response:
        """
        ...
        headers = self.__ProcessHeaders(headers)
        # Ответ.
        Response = self.__Client.post(url, params=params, headers=headers, cookies=cookies, data=data, json=json,
                                      follow_redirects=self.__Config.redirecting)

        return Response

    def __requests_GET(self, url: str, params: dict | None = None, headers: dict | None = None,
    ...
    def __requests_POST(self, url: str, params: dict | None = None, headers: dict | None = None,
                        cookies: dict | None = None, data: any = None, json: dict | None = None) -> requests.Response:
        """
        ...
        headers = self.__ProcessHeaders(headers)
        # Ответ.
        Response = self.__Session.post(url, params=params, headers=headers, cookies=cookies, data=data, json=json,
                                       proxies=self.__GetProxy(), allow_redirects=self.__Config.redirecting)
    ...
    def add_proxy(self, protocol: Protocols, host: str, port: int | str, login: str | None = None,
            ...
            while IsLoaded == False:
                ...
                try:
                    ...
                    Result = self.__Browser.execute_async_script(
                        script) if use_async == True else self.__Browser.execute_script(script)
    ...
    def load(self, url: str, tries: int = 3) -> WebResponse:
        ...
        while Try < tries and Response.status_code != 200:
            ...
            try:
                ...
                if self.__Logging == True: logging.error(
                    "[SELENIUM-LOAD] Description: \"" + str(ExceptionData).split("\n")[0] + "\".")
    ...
    def get(self, url: str, params: dict | None = None, headers: dict | None = None, cookies: dict | None = None,
            tries: int = 1) -> requests.Response | httpx.Response:
        """
        ...
        while Try < tries and Response.status_code != 200:
            ...
            try:
                ...
                # Запись в лог ошибки: не удалось выполнить запрос.
                if self.__Logging == True: logging.error(
                    f"[{LibName}-GET] Description: \"" + str(ExceptionData).split("\n")[0] + "\".")

        return Response

    def post(self, url: str, params: dict | None = None, headers: dict | None = None, cookies: dict | None = None,
             data: any = None, json: dict | None = None, tries: int = 1) -> requests.Response | httpx.Response:
        """
        ...
        while Try < tries and Response.status_code != 200:
            ...
            try:
                ...
                # Запись в лог ошибки: не удалось выполнить запрос.
                if self.__Logging == True: logging.error(
                    f"[{LibName}-POST] Description: \"" + str(ExceptionData).split("\n")[0] + "\".")
